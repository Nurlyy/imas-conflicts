...
for($i=1;$i<=2;$i++){
	...
	foreach($categories as $cat){
		...
		$counts=array();
		$url= "https://rest.imas.kz/dashboards/statproject?date=".$date."&p_id=".$p_id."&token=1qEeeotYWbSY";
		// echo $url.'<br><br>';
		$result=json_decode(get_web_page($url),true);
		$smi_count=$result['smi_count'];
		$social_count=$result['social_count'];
		$pos_count=$result['pos_count'];
		$neg_count=$result['neg_count'];
		$neu_count=$result['neu_count'];
		$query_insert = "INSERT IGNORE INTO v_40047_special.`mvd_dynamic`(`date`, `smi`, `social`, `pos`, `neg`, `neu`, `category_id`) VALUES ('".$date."',".$smi_count.",".$social_count.",".$pos_count.",".$neg_count.",".$neu_count.",".$category_id.") ON DUPLICATE KEY UPDATE smi=".$smi_count.", social=".$social_count.", pos=".$pos_count.", neg=".$neg_count.", neu=".$neu_count;
