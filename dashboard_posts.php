...
foreach($categories as $cat){
	$category_id=$cat['id'];
	$p_id=$cat['s_id'];
	$url= "https://rest.imas.kz/dashboards/postsnewproject?p_id=".$p_id."&token=1qEeeotYWbSY";
	echo $url.'<br>';
	...
	foreach($result as $element):
		...
		$date=$element['date'];
		$title='';
		$text=htmlspecialchars(addslashes($element['text']));
		$resource_name=htmlspecialchars(addslashes($element['resource_name']));
		$res_link=$element['link'];
		$query_insert = "INSERT IGNORE INTO v_40047_special.`mvd_live_posts`(`news_id`, `not_date`, `date`, `title`, `text`, `resource_name`, `res_link`, `category_id`) VALUES (".$news_id.", '".$not_date."', ".$date.", '".$title."', '".$text."', '".$resource_name."', '".$res_link."', ".$category_id.")";
