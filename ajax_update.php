...
switch($_GET['type']){
	...
	case 'top_smi':{
		...
		$sql_top_smi= mysqli_query($connect,"Select * From dashboard_new_top_smi order by news_count desc limit 15")or die(mysqli_error($connect));
		...
		$sql_top_smi= mysqli_query($connect,"Select * From dashboard_new_top_smi where res_id = ".$res_id)or die(mysqli_error($connect));
		/*while($table_top_smi = mysqli_fetch_array($sql_top_smi)){
			$top_smi_array[] = $table_top_smi;
		}
		foreach($top_smi_array as $top_smi){
	...
	case 'top_social':{
		$res_id = (int) $_GET['res_id'];
		$sql_top_social= mysqli_query($connect,"Select * From dashboard_new_top_social order by news_count desc limit 15")or die(mysqli_error($connect));
		...
		$sql_top_social= mysqli_query($connect,"Select * From dashboard_new_top_social where res_id = ".$res_id)or die(mysqli_error($connect));
		/*while($table_top_social = mysqli_fetch_array($sql_top_social)){
			$top_social_array[] = $table_top_social;
		}
		foreach($top_social_array as $top_social){
	...
	case 'news_items':{
		...
		$sql_news_items= mysqli_query($connect,"Select * From dashboard_new_items WHERE date>".$time." AND date<=".time()." ORDER BY date")or die(mysqli_error($connect));
	...
	case 'news_posts':{
		$time=mktime(date("H"),date("i")-5,date("s"),date("m"),date("d"),date("Y"));
		$sql_news_posts= mysqli_query($connect,"Select * From dashboard_new_posts WHERE date>".$time." AND date<=".time()." ORDER BY date")or die(mysqli_error($connect));
